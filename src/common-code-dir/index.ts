      ...
      >(
        ...
        R.filter<string>(
          (innerPath: string) =>
            ...
            ) && !R.any(R.includes(R.__, innerPath), R.append('/common/', params.extraCommonDirNames || [])),
        ),
        ...
        R.map((innerPath) =>
          ...
          >(
            ...
            R.map((importPath) =>
              path.resolve(
                (importPath || '').match(/^[a-z]/i)
                  ? params.baseImportPath || ''
                  : innerPath.replace(/\/index\.tsx/, ''),
                importPath || '/',
              ),
            ),
        ...
        R.map<string, Array<string>>((includePath) =>
          R.filter(
            (innerPath) =>
              !R.isEmpty(R.match(/(js|jsx|ts|tsx)$/i, innerPath)) &&
